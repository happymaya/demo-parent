...
import cn.happymaya.support.exception.OrmException;
...
public class BeanUtils {
    ...
    private static Object bindListOrSet(ParameterizedType pt, String str) {
        ...
        if (ata instanceof Class) {
            ...
            if (ataClazz.equals(String.class)) return convert(listOfStr, clazz, s -> {
                return s;
            });
            if (ataClazz.equals(Long.class) || clazz.equals(long.class)) return convert(listOfStr, clazz, s -> {
                return new Long(s);
            });
            if (ataClazz.equals(Integer.class) || clazz.equals(int.class)) return convert(listOfStr, clazz, s -> {
                return new Integer(s);
            });
            if (ataClazz.equals(Double.class) || clazz.equals(double.class)) return convert(listOfStr, clazz, s -> {
                return new Double(s);
            });
            if (ataClazz.equals(Float.class) || clazz.equals(float.class)) return convert(listOfStr, clazz, s -> {
                return new Float(s);
            });
            if (ataClazz.equals(Short.class) || clazz.equals(short.class)) return convert(listOfStr, clazz, s -> {
                return new Short(s);
            });

            if (ataClazz.equals(Date.class) && listOfStr.get(0).length() == 10) return convert(listOfStr, clazz, s -> {
                return DateUtils.getDate(s, "yyyy-MM-dd");
            });
            if (ataClazz.equals(Date.class)) return convert(listOfStr, clazz, s -> {
                return DateUtils.getDate(s, "yyyy-MM-dd HH:mm:ss");
            });

            if (EntityUtils.isEntity(ataClazz)) return EntityUtils.bindListOrSetOfEntity((Class<Entity>) ataClazz, str);
        } else {
     ...
     * @return Collection<T>
     */
    private static <T> Collection<T> convert(List<String> listOfStr, Class<?> clazz, NewInstance<T> instance) {
